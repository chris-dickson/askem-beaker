...
def start_chromadb(docker=False):
    ...
    if docker:
        ...
        chroma_client = chromadb.PersistentClient(path="/home/jupyter/chromadb_functions")
                ...
                if inspect.isclass(attribute):
                    ...
                    try:
                        ...
                        docs[full_name] = {
                            ...
                            'docstring': docstring,
                            'source_code': source_code,
                        }  # TODO: replace source code with function signature?
                    except:
                        continue
    if len(docs) > 0:
        ...
        metadatas = [
            {
                "function_name": docs[doc]['full_name'],
                "docstring": docs[doc]['docstring'],
                "source_code": docs[doc]['source_code'],
            }
            for doc in docs
        ]
...
def process_ask_gpt_in_parallel(prompts, prompt_names, max_workers=8, model="gpt-3.5-turbo-1106", **kwargs):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(ask_gpt, prompt, model, **kwargs): name for prompt, name in zip(prompts, prompt_names)
        }
...
def get_expanded_descriptions(docs, max_workers=8, model="gpt-3.5-turbo-1106"):
    ...
    prompts = {
        doc: check_and_trim_tokens(
            prompt.format(
                func_name=docs[doc]['full_name'], docstring=docs[doc]['docstring'], source_code=docs[doc]['source_code']
            ),
            model,
        )
        for doc in docs
