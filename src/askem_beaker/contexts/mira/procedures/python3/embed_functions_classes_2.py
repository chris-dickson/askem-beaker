    ...
    for file in files:
        if file.endswith('.py'):
            ...
            if functions_classes:
                print_functions = [
                    {'name': functions_class[0], 'source_code': functions_class[1], 'doc_string': functions_class[2]}
                    for functions_class in functions_classes
                ]
                documents.extend(
                    [
                        (json.dumps(print_function), {'file_path': file, 'name': print_function['name']})
                        for print_function in print_functions
                    ]
                )
...
def embed_functions_and_classes(function_dir, collection_name="function_index", library_name="mira"):
    ...
    collection.add(documents=document_texts, metadatas=metadatas, ids=ids)


def query_functions_classes(
    query, collection_name="function_index", path="/home/jupyter/chromadb_functions", n_results=5
):
